...
/* Strat part */
#define STRAT_SIZE 13

const Action bleu_strat_model[STRAT_SIZE] = {
    create_action(0.0, 0.0, NEXT_POSITION_DELAY, ACTION_ERROR_AUTH), // SERVO ON
    create_action(140.0, 0.0, NEXT_POSITION_DELAY, ACTION_ERROR_AUTH),
    create_action(224.0, 42.0, NEXT_POSITION_DELAY, ACTION_ERROR_AUTH),
    create_action(0.0, 0.0, NEXT_POSITION_DELAY, ACTION_ERROR_AUTH), // SERVO OFF
    create_action(206.0, 59.0, NEXT_POSITION_DELAY, ACTION_ERROR_AUTH),
    create_action(206.0, 47.0, NEXT_POSITION_DELAY, ACTION_ERROR_AUTH),
    create_action(230.0, 1.0, NEXT_POSITION_DELAY, ACTION_ERROR_AUTH),
    create_action(0.0, 0.0, NEXT_POSITION_DELAY, ACTION_ERROR_AUTH), // SERVO ON
    ...
    create_action(40.0, 47.0, NEXT_POSITION_DELAY, ACTION_ERROR_AUTH),
    create_action(9.0, 15.0, NEXT_POSITION_DELAY, ACTION_ERROR_AUTH)};
Action bleu_return_position = create_action(9.0, 15.0, NEXT_POSITION_DELAY, ACTION_ERROR_AUTH);
...
const Action green_strat_model[STRAT_SIZE] = {
    ...
    create_action(40.0, -47.0, NEXT_POSITION_DELAY, ACTION_ERROR_AUTH),
    create_action(9.0, -15.0, NEXT_POSITION_DELAY, ACTION_ERROR_AUTH)};
Action green_return_position = create_action(9.0, -15.0, NEXT_POSITION_DELAY, ACTION_ERROR_AUTH);
...
void loop()
{
  ...
  if (!((millis() - start_time) < STOP_MOTORS_DELAY || start_time == -1)){
    while (true){
      ...
      {
        rolling_basis_ptr->left_motor->_threshold_pwm_value = 0;
        rolling_basis_ptr->right_motor->_threshold_pwm_value = 0;
        rolling_basis_ptr->left_motor->set_motor(1, 0);
        rolling_basis_ptr->right_motor->set_motor(1, 0);
        rolling_basis_ptr->max_pwm = 0;
      }
    }
  }
}
    ...
    if (digitalRead(pin_on_off))
    {
      ...
      */
      ...
      if (action_index < STRAT_SIZE)
      {
        ...
        if(action_index == 0 || action_index == 7){
        ...
        else if (action_index == 3 || action_index == 9){
          r_servo.write(200);
          l_servo.write(0);
          action_index++;
        }
          ...
          /*
          if (action_index == 0 || action_index == 2 || action_index == 3 || action_index == 6)
