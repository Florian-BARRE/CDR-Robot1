...
void Basic_Action::handle(Point current_point, Ticks current_ticks, Rolling_Basis_Ptrs *rolling_basis_ptrs)
{
    ...
    // Handle the action 
    if (this->is_computed && (this->state == in_progress || this->state == finished))
        this->step_action->handle(current_ticks, rolling_basis_ptrs);
...
{   
    ...
    */
    ...
    float theta = fmod((theta_dif - current_point.theta + PI), (2 * PI)) - PI;
