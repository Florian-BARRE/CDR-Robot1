...
void Go_To::compute(float current_x, float current_y, float current_theta, long current_right_ticks, long current_left_ticks, Rolling_Basis_Params *rolling_basis_params)
{   
    //  Go to is simple move
    //  1°) rotate to be in front of the target point
    //  2°) go forward to the target point
    this->alloc_memory(2);
    this->basic_actions[0] = new Get_Orientation(
        this->target_x,
        this->target_y,
        this->dir,
        this->speed,
